...
plot(datetime,exdata2[,5],type='l',ylab="Voltage")
plot(datetime,exdata2[,7],type='l',
     ylab="Energy sub metering", xlab="")
lines(datetime,exdata2[,8],col=2)
lines(datetime,exdata2[,9],col=4)
legend("topright",legend=names(exdata2[7:9]),lty=1,lwd=1,col=c(1,2,4),bty='n')
plot(datetime,exdata2[,4],type='l',ylab=names(exdata2)[4])
dev.off()
